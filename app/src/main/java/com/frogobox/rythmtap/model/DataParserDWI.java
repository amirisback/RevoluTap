...
public class DataParserDWI {
	...
	// TODO
	private static void addNotes(Queue<DataNote> notes, DataNote.NoteType nt,
				...
				if (c == '!') { // Hold
					...
					if (pitchA != -1) {
						for (DataNote n : notes) {
							if (n.column == pitchA) {
								n.noteType = DataNote.NoteType.HOLD_START;
					...
					while (!notes.isEmpty()) {
						...
						// Holds
						if (!holds || randomize) {
							if (n.noteType.equals(DataNote.NoteType.HOLD_START)) {
								n.noteType = DataNote.NoteType.TAP_NOTE;
								nd.addNote(n);
							} else if (!n.noteType.equals(DataNote.NoteType.HOLD_END)) { // Don't add hold ends
								nd.addNote(n);
							}
						} else { 
							nd.addNote(n);
					...
					if (step) {
						DataNote.NoteType ntA = DataNote.NoteType.TAP_NOTE;
						DataNote.NoteType ntB = DataNote.NoteType.TAP_NOTE;
						// Note
						rand.setupNextLine();
						if (pitchA != -1) {
							...
							while (it.hasNext()) {
								Integer h = it.next();
								if (h.equals(pitchA)) {
									ntA = DataNote.NoteType.HOLD_END;
									it.remove();
								}
							}
							if (jumps || activeHolds.isEmpty() || ntA.equals(DataNote.NoteType.HOLD_END)) {
						...
						if (pitchB != -1) {
							...
							while (it.hasNext()) {
								Integer h = it.next();
								if (h.equals(pitchB)) {
									ntB = DataNote.NoteType.HOLD_END;
									it.remove();
								}
							}
							if ((jumps || ntB.equals(DataNote.NoteType.HOLD_END))) {
